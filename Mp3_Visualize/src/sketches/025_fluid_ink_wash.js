...
export default class FluidInkWashSketch {
  constructor(container) {
    ...
    this.loadedSeed = -1;/**
 * src/sketches/025_fluid_ink_wash.js
 * - [025호 수묵 잉크 블룸 Ver 15.0 - Sharp & High Contrast Wash]
 * - 흐릿한 전면 블러 제거 ➔ 또렷하고 선명한 2D/3D 수묵 베일 윤곽선 복원
 * - 잉크 농도(Alpha) & 외곽 마름선 선명도 대폭 상승 ➔ 깊이 있는 겹침 중첩
 */
...
export default class FluidInkWashSketch {
  constructor(container) {
    ...
    this.time = 0;
    this.version = "025호 수묵 잉크 블룸 Ver 15.0 (High Contrast)";
  ...
  // =========================================================================
  // 🖌️ 선명한 수묵 베일 렌더링 (No Blur, High Contrast)
  // =========================================================================
  drawFluidVeil(ctx, veil, time, scatterMotion, baseColorRgb, edgeRgb, vocalVol, drumVol, bassVol, otherVol, isDark) {
    ...
    // 💡 [핵심]: 흐릿함을 유발하던 filter blur 제거 -> 선명한 벡터 곡선 표현
    ctx.filter = 'none';

    // 1. 또렷한 수묵 면 채우기 (Alpha 상승)
    const fillAlpha = isDark ? (0.35 + bassVol * 0.2) : (0.28 + bassVol * 0.18);
    ctx.fillStyle = `rgba(${baseColorRgb}, ${fillAlpha})`;
    ctx.fill();

    // 2. 선명한 외곽 마름선 (Ink Edge Bleed)
    const strokeAlpha = isDark ? (0.75 + drumVol * 0.25) : (0.65 + drumVol * 0.25);
    ctx.strokeStyle = `rgba(${edgeRgb}, ${strokeAlpha})`;
    ctx.lineWidth = 1.5 + drumVol * 1.5;
    ctx.stroke();
  ...
  update(audioData) {
    ...
    let bgColor = "#f4f1ea";
    let isDark = false;
    let getColors = (idx) => ({ base: "20, 24, 32", edge: "2, 3, 5" });

    if (colorStyle === 'monochrome' || colorStyle === 'earth') {
      bgColor = "#f4f1ea";
      isDark = false;
      getColors = (idx) => ({
        base: idx % 2 === 0 ? "18, 22, 30" : "35, 42, 55",
        edge: "2, 4, 8"
      });
    ...
    window.sketchDiagnostics = {
      fps: 60,
      particleCount: `9 High-Contrast Sharp Veils`,
      isCovering: true,
      activeFunction: `FluidInkWash[HighContrast_${colorStyle.toUpperCase()}]`
    };
  }
